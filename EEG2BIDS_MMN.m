...
if length(varargin) == 1
    ...
    if isstruct(data_dir)
        assert(~isstruct(data_dir),sprintf('No session is specified, however, the script %s has %i session path(s)\n',this_file_path,length(fieldnames(data_dir))))
    else
        ses = {'None'};
        sub.(ses{1}) = varargin{1};
...
copyfile(strcat(this_file_path,'.m'), fullfile(code_dir,strcat(name,'.m')), 'f')
